...
// Delegate functions
protocol BluetoothSerialDelegate1 {
    // ** Required **
...
extension BluetoothSerialDelegate1 {
    func serialDidReceiveString(_ message: String) {}
    func serialDidReceiveBytes(_ bytes: [UInt8]) {}
    func serialDidReceiveData(_ data: Data) {}
    func serialDidReadRSSI(_ rssi: NSNumber) {}
...
final class BluetoothSerial: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var delegate1: BluetoothSerialDelegate1!
    ...
    init(delegate1: BluetoothSerialDelegate1) {
        super.init()
    ...
    func startScan() {
        ...
        for peripheral in peripherals {
            delegate1.serialDidDiscoverPeripheral(peripheral, RSSI: nil)
        }
    }
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        delegate1.serialDidDiscoverPeripheral(peripheral, RSSI: RSSI)
    }
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        delegate1.serialDidConnect(peripheral)
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        // send it to the delegate
        delegate1.serialDidDisconnect(peripheral, error: error as NSError?)
    }
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        ...
        // just send it to the delegate
        delegate1.serialDidFailToConnect(peripheral, error: error as NSError?)
    }
    ...
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        // send it to the delegate
        delegate1.serialDidChangeState()
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in service.characteristics! {
            if characteristic.uuid == characteristicUUID {
                ...
                delegate1.serialIsReady(peripheral)
            }
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        // first the data
        delegate1.serialDidReceiveData(data!)
        
        // then the string
        if let str = String(data: data!, encoding: String.Encoding.utf8) {
            delegate1.serialDidReceiveString(str)
        } else {
        ...
        delegate1.serialDidReceiveBytes(bytes)
    }
    
    func peripheral(_ peripheral: CBPeripheral, didReadRSSI RSSI: NSNumber, error: Error?) {
        delegate1.serialDidReadRSSI(RSSI)
    }
}
